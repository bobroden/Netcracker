...
export class ChangeFormComponent implements OnInit {
	constructor(private studentsService: StudentsService, private cd: ChangeDetectorRef, private ar: ActivatedRoute, private router: Router, private store$: Store) {}
	...
	change(): void {
		if (this.changeCheckedFullname === "VALID" && this.changeCheckedDate === "VALID" && this.changeCheckedScope === "VALID") {
			...
			this.studentsService.change(this.student, this.change_index, i);
			if (this.studentsService.studentsList === undefined) {
			...
			this.store$.dispatch(StoreActions.sendLastSurname({lastSurname: this.student.Surname}));
			this.store$.dispatch(StoreActions.sendLastDate());
