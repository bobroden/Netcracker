...
export class ServerService {
	...
	getAll(): Observable<Object> {
		const b = this.http.get("http://localhost:3000/students");
		b.subscribe(response => {
			this.studentsList = response;
			this.originalStudentsList = response;
	...
	change(student: Student, id: number): void {
		...
		this.originalStudentsList.splice(id, 1, student);
